setwd('c:\\easy_r')
library(ggmap)
library(stringr)

loc <- read.csv('강북구주차장.csv',header=T)
loc
center <- c(mean(loc$LON), mean(loc$LAT))
kor <-get_map(center,zoom=11, maptype = 'roadmap')
kor.map <- ggmap(kor) +geom_point(data=loc,aes(x=LON,y=LAT),
                                  size=0.7,alpha=0.7,color='red')
kor.map+geom_text(data=loc, aes(x= LON, y= LAT+0.005,label=주차장명),size=0.7)
ggsave('c:\\easy_r/gangdog.png',dpi=500)


loc2 <- str_sub(loc$주차장명,start=-2,end=-2)
loc2
colors <- c()
for (i in 1:length(loc2)){
  if (loc2[i]=='구'){
    colors<- c(colors,'red')}
  else {
    colors <- c(colors,'blue')}
}
kor <-get_map(center,zoom=11, maptype = 'roadmap')
kor.map <- ggmap(kor) +geom_point(data=loc,aes(x=LON,y=LAT),
                                  size=0.7,alpha=0.7,color='red')
kor.map+geom_text(data=loc, aes(x= LON, y= LAT+0.005,label=주차장명),size=0.7)
ggsave('c:\\easy_r/gangdog2.png',dpi=500)